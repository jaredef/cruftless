...
- **diff-prod**: 112 fixtures across every resolution layer. 58 PASS / 54 FAIL at the runtime-semantics probe. Fixtures at `scripts/diff-prod/fixtures/`; runner at `scripts/diff-prod/run-all.sh`. Each fixture runs under both `cruft` and `bun` and diffs stdout byte-for-byte. Failures compress into 9 named mechanism gaps. The bidirectional Pin-Art probing methodology at `pilots/apparatus/diff-prod/high-resolution-bidirectional-pin-art-probing.md` describes how to read the fixture results as both forward constraints (the engine must satisfy) and backward invariant discovery (the engine reveals implicit commitments).
- **test262-sample**: 84.8% runnable pass rate (6,377 PASS / 1,139 FAIL on the curated representative sample of 7,598 tests). Runner at `scripts/test262-sample/run-sample.sh`; sample paths at `scripts/test262-sample/sample-paths.txt` target the surface real Node packages exercise.
