...
- [`apparatus/docs/audit-ledger.md`](apparatus/docs/audit-ledger.md) — append-only record of apparatus-tier audits: scope, method, findings, authored actions, surfaced-but-not-acted findings. An audit session that produces classifications or authorizes ledger flips owes the apparatus an audit-ledger entry, not merely a Telegram report.
- [`apparatus/docs/findings-ledger.md`](apparatus/docs/findings-ledger.md) — append-only aggregator of substrate empirical findings across locales; per-locale `findings.md` inventory + cross-locale recurrence patterns + bidirectional back-references to `apparatus/docs/predictive-ruleset.md`. A substrate rung that surfaces a generalizable finding owes the apparatus an aggregator entry.
- [`apparatus/docs/findings-disposition-protocol.md`](apparatus/docs/findings-disposition-protocol.md) — apparatus self-applying its 5-phase substrate-shaped-work pipeline at the finding tier; 8 disposition candidates (integrate-existing-arc / integrate-scaffolded-arc / lift-to-new-arc / promote-to-standing-rule / relocate-to-apparatus-pilot / lattice-meet-annotation / defer-with-cross-reference / close-as-locale-singleton). Paired with `orphan-disposition-protocol.md` as the engagement-tier instance of Phase 5 chapter-close-inspect at the finding subsumption boundary.
...
### Findings-disposition protocol (engagement-tier Phase 5 instance, paired with orphan-disposition)

When a findings audit lands (or a new finding is authored with cross-locale recurrence ≥2), run the findings-disposition protocol at `apparatus/docs/findings-disposition-protocol.md`. The protocol is the apparatus self-applying its 5-phase substrate-shaped-work pipeline at the finding tier — findings are the cells; arcs and standing rules are the lifted substrates. Per finding: recover (M, T, I, R) at finding tier; apply Rule 11 5-axis at finding tier; baseline-inspect against `apparatus/docs/predictive-ruleset.md` + `pilots/rusty-js-jit/findings.md` + per-locale `findings.md`; test eight disposition candidates in order (integrate-existing-arc / integrate-scaffolded-arc / lift-to-new-arc / promote-to-standing-rule / relocate-to-apparatus-pilot / lattice-meet-annotation / defer-with-cross-reference / close-as-locale-singleton); first that fits is the disposition. Surface cross-finding patterns for standing-rule promotion per Doc 727 §X.

The two protocols compose at engagement-wide chapter-close: orphan-disposition first (locales without arcs), findings-disposition second (findings without arcs or standing rules). A locale dispositioned to "scaffold-new-arc" by orphan-disposition becomes a candidate receiver for "integrate-into-scaffolded-arc" by findings-disposition at the next cycle.
...
- **test262-sample**: **88.7% runnable pass rate** (6816 PASS / 865 FAIL / 16 SKIP on the curated representative sample of 7750 tests; 7697 emitted; 7681 runnable) per 2026-05-30 canonical run. **+4.4 points over the 2026-05-27 canonical (84.3%, 6443/7647)** reflecting today's session work: TABSC-EXT 0 byte-storage architectural rectification + TABSC-EXT 1 DataView coercion-faithfulness + TAECSF cascade-revival + rules 17-22 consolidation + multi-step apparatus discipline. Runner at `scripts/test262-sample/run-sample.sh`; sample paths at `scripts/test262-sample/sample-paths.txt` target the surface real Node packages exercise.
- **test262-full**: **67.6% runnable pass rate** (32,460 PASS / 15,537 FAIL / 4,933 SKIP at last full run; +3.7 points over the prior canonical 63.9% baseline). Categorizer at `pilots/apparatus/test262-categorize/derived/src/bin/full_pinart.rs` projects failures onto 323 Pin-Art coordinates across 12 resolver instances. Latest results at `pilots/apparatus/test262-categorize/full-suite/results/test262-full-2026-05-28-123833-p2/`.
