...
Every workstream is a **locale**: a directory containing `seed.md` + `trajectory.md` per Doc 581, and optionally an `analysis.md` cross-referencing the locale's scope against empirical diff-prod results. The seed names the telos, apparatus, methodology, carve-outs, and resume protocol; the trajectory records substrate moves in time order; the analysis maps the locale's declared scope to the 112-fixture diff-prod suite's PASS/FAIL vector and names which mechanism gaps affect the locale. The three-file pattern recurs at every substrate depth per Doc 733 (cross-tier fractal recurrence) and within tiers per Doc 737 (sub-workstreams with multi-rung shape spawn nested locales at deeper coordinates).

### The analysis.md file

The `analysis.md` is a contingent element: not every locale requires one, but when present it provides empirical grounding that an agent should consult before beginning substrate work. The file cross-references the locale's seed-declared scope against the diff-prod fixture suite (112 fixtures, 58 PASS / 54 FAIL) and identifies:

- **Which fixtures are relevant** to the locale's scope (typically 2-6)
- **Whether the engine is correct** (PASS), **divergent** (FAIL), or **untested** at the locale's surface
- **Which of the 9 named mechanism gaps** affect the locale's telos

An agent picking up work on a locale should read `analysis.md` after `seed.md` + `trajectory.md` to understand the empirical state before proposing substrate moves. For locales where diff-prod is not the right measurement instrument (JIT/shapes measured by CRB; TS-resolve measured by TCC/TXC; infrastructure locales outside ECMA-262), the analysis.md notes this and directs the agent to the appropriate apparatus.

**Discipline**: when a substrate move flips a diff-prod fixture from FAIL to PASS, update the locale's `analysis.md` to reflect the new state. When spawning a new locale, write an `analysis.md` that maps its seed scope to the current fixture results.
