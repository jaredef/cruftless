...
- **test262-sample**: 84.8% runnable pass rate (6,377 PASS / 1,139 FAIL on the curated representative sample of 7,598 tests, parallel-branch canonical instrument). Runner at `scripts/test262-sample/run-sample.sh`; sample paths at `scripts/test262-sample/sample-paths.txt` target the surface real Node packages exercise. **Re-measurement against current main pending** (this number is the last recorded canonical sample read; post-array-exotic-substrate + JSON-serialization-carve-out + super-reference-lowering arc closures have not been reflected via a fresh sample run).
