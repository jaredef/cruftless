...
"#,
    ...
    assert!(
        stdout.contains("identity42=42"),
        "expected 'identity42=42' in stdout; got:\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        stdout.contains("keys=lots"),
...
fn cli_install_then_run() {
    ...
    assert!(
        install_stdout.contains("+ lodash@4.17.21"),
        "expected '+ lodash@4.17.21' in stdout; got: {install_stdout}"
    );
    assert!(
        project.join("node_modules/lodash/package.json").exists(),
        "lodash should be installed"
    );
    assert!(
        project.join("cruftless-lock.json").exists(),
