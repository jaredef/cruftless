...
pub fn install(rt: &mut Runtime) {
    ...
    register_method(rt, os, "endianness", |_rt, _args| {
        Ok(Value::String(Rc::new(if cfg!(target_endian = "little") {
            "LE".into()
        } else {
            "BE".into()
        })))
    ...
    register_method(rt, os, "cpus", |rt, _args| {
        ...
        for i in 0..count {
            ...
            rt.object_set(
                cpu,
                "model".into(),
                Value::String(Rc::new("Unknown CPU".into())),
            );
    ...
    register_method(rt, os, "totalmem", |_rt, _args| {
        Ok(Value::Number(8.0 * 1024.0 * 1024.0 * 1024.0))
    });
    register_method(rt, os, "freemem", |_rt, _args| {
        Ok(Value::Number(4.0 * 1024.0 * 1024.0 * 1024.0))
    });
    ...
    register_method(rt, os, "loadavg", |rt, _args| {
        let arr = rt.alloc_object(rusty_js_runtime::value::Object::new_array());
        for i in 0..3 {
            rt.object_set(arr, i.to_string(), Value::Number(0.0));
        }
