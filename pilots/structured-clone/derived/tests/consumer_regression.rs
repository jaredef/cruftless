...
fn consumer_immer_shared_reference_survives_clone() {
    ...
    let id_a = match entries[0].1 {
        Value::Ref(id) => id,
        _ => panic!(),
    };
    let id_b = match entries[1].1 {
        Value::Ref(id) => id,
        _ => panic!(),
    };
    assert_eq!(
        id_a, id_b,
        "immer requires shared-ref identity preservation"
    );
}
...
fn consumer_postmessage_circular_reference_roundtrip() {
    ...
    let HeapObject::Object(entries) = cloned_heap.get(new_id) else {
        panic!()
    };
    let self_ref = match entries[0].1 {
        Value::Ref(id) => id,
        _ => panic!(),
    };
...
fn consumer_lodash_migration_set_order_preserved() {
    ...
    assert_eq!(
        items,
        &vec![Value::Number(7.0), Value::Number(3.0), Value::Number(11.0)]
    );
}
...
fn consumer_typed_array_views_share_cloned_buffer() {
    ...
    let v1_id = match items[0] {
        Value::Ref(id) => id,
        _ => panic!(),
    };
    let v2_id = match items[1] {
        Value::Ref(id) => id,
        _ => panic!(),
    };
    let HeapObject::TypedArrayView { buffer: b1, .. } = cloned_heap.get(v1_id) else {
        panic!()
    };
    let HeapObject::TypedArrayView { buffer: b2, .. } = cloned_heap.get(v2_id) else {
        panic!()
    };
    assert_eq!(
        b1, b2,
        "two views over the same buffer must share the cloned buffer"
    );
}
