# spec-builtins-wrong-result — Seed
...
```
runtime/spec-builtins :: E3/intrinsic-object:ecma-262 :: value-semantics/wrong-result :: assertion/expected-mismatch
```
...
**Heuristics §IV classification**: B/D (Shared-Upstream / Intrinsic Semantics)

Built-in constructors and prototypes produce wrong values. Long tail of AggregateError/Atomics/etc. (122 families). Heuristics §IV.B: each row needs 5+ shared-mechanism examples before substrate work; may need apparatus refinement (abstract-op mapping per §IX) first.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 389-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (389 fails)
