...
fn function_typed_body() {
    if let Stmt::FunctionDecl { body, .. } =
        first_stmt("function f(x) { let y = x + 1; return y; }")
    {
        assert_eq!(body.len(), 2);
        assert!(matches!(&body[0], Stmt::Variable(_)));
...
fn function_recursive_body() {
    ...
    if let Stmt::FunctionDecl { body, .. } =
        first_stmt("function outer() { function inner() { return 1; } return inner; }")
    {
        assert_eq!(body.len(), 2);
...
fn switch_statement() {
    if let Stmt::Switch {
        ...
        cases,
        ..
    } = first_stmt("switch (x) { case 1: a(); break; case 2: b(); break; default: c(); }")
    {
...
fn typed_module_body() {
    ...
    assert!(matches!(
        &m.body[1],
        ModuleItem::Statement(Stmt::Variable(_))
    ));
    assert!(matches!(
        &m.body[2],
        ModuleItem::Statement(Stmt::Expression { .. })
    ));
            ...
            for (i, expected) in ["a", "b"].iter().enumerate() {
                if let Some(e) = &arr.elements[i] {
                    if let BindingPattern::Identifier(id) = &e.target {
                        ...
                        panic!("expected ident at {}", i);
                    }
                } else {
                    panic!("expected element at {}", i);
                }
            }
            // rest
...
fn function_with_two_destructure_params() {
    if let Stmt::FunctionDecl { params, .. } = first_stmt("function f({a, b}, [c, d]) {}") {
        assert_eq!(params.len(), 2);
        if let BindingPattern::Object(_) = &params[0].target {
        } else {
            panic!("expected object pattern param 0");
        }
        if let BindingPattern::Array(_) = &params[1].target {
        } else {
            panic!("expected array pattern param 1");
        }
        let n0: Vec<String> = params[0]
            .target
            .collect_names()
            .into_iter()
            .map(|n| n.name.clone())
            .collect();
        let n1: Vec<String> = params[1]
            .target
            .collect_names()
            .into_iter()
            .map(|n| n.name.clone())
    ...
    if let Stmt::Expression { expr, .. } = first_stmt("`hello ${x} world ${y}!`;") {
        ...
        } = expr
        {
            ...
            if let Expr::Identifier { name, .. } = &expressions[0] {
                ...
                panic!("expected ident x");
            }
            if let Expr::Identifier { name, .. } = &expressions[1] {
                assert_eq!(name, "y");
            } else {
                panic!("expected ident y");
            }
        } else {
            panic!("expected template literal, got {:?}", expr);
        }
    } else {
