...
fn import_named() {
    ...
    assert_eq!(
        ...
        vec![
            ImportEntry {
                module_request: "pkg".into(),
                import_name: ImportName::Single("a".into()),
                local_name: "a".into()
            },
            ImportEntry {
                module_request: "pkg".into(),
                import_name: ImportName::Single("b".into()),
                local_name: "c".into()
            },
        ]
    );
...
fn import_string_literal_module_export_name() {
    ...
    assert_eq!(
        ...
        vec![ImportEntry {
            module_request: "pkg".into(),
            import_name: ImportName::Single("m-search".into()),
            local_name: "ms".into()
        },]
    );
}
...
fn export_string_literal_export_name() {
    ...
    assert_eq!(
        m.local_export_entries,
        vec![ExportEntry {
            export_name: Some("m-search".into()),
            module_request: None,
            import_name: None,
            local_name: Some("x".into())
        },]
...
fn export_default_expression() {
    ...
    assert_eq!(
        m.local_export_entries[0].export_name.as_deref(),
        Some("default")
    );
    assert_eq!(
        m.local_export_entries[0].local_name.as_deref(),
        Some("*default*")
    );
}
...
fn export_default_named_function() {
    ...
    if let ModuleItem::Export(ExportDeclaration::Default { body, .. }) = &m.body[0] {
        if let DefaultExportBody::HoistableFunction {
            name,
            is_async,
            is_generator,
            ..
...
fn export_const_declaration() {
    ...
    let names: Vec<&str> = m
        ...
        .iter()
        .map(|e| e.export_name.as_deref().unwrap())
        .collect();
...
fn statement_or_decl_passes_through() {
    ...
    let exports: Vec<_> = m
        .body
        .iter()
        .filter(|i| matches!(i, ModuleItem::Export(_)))
        .collect();
