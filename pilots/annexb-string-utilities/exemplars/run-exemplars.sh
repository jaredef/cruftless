#!/usr/bin/env bash
# AnnexB-string-utilities exemplar runner. Iterates exemplars.txt, runs
# each via the test262 harness wrapper against cruft, prints aggregate
# pass/fail and per-target breakdown.
set -uo pipefail
...
echo "AnnexB-string-utilities: PASS=$PASS FAIL=$FAIL / $TOTAL  ($PCT%)"
echo "--- fail breakdown ---"
for f in "${FAILS[@]:-}"; do echo "$f" | awk -F/built-ins/ '{print $2}' | awk -F/ '{print $1}'; done | sort | uniq -c | sort -rn
