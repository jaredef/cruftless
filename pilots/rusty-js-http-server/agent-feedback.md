...
6. **HS-EXT 8b broadened Express evidence.** A tracked opt-in fixture now proves Express middleware chaining, `next()`, route params, query parsing, response headers, and `status(201).send(...)` over the same live HTTP server path. With `CRUFTLESS_EXPRESS_FIXTURE_ROOT=/tmp/cruftless-hs-express-minimal`, the HTTP-server lane is 6/6 green.
7. **Binary bodies and slow/chunked reads remain later-scope gaps.** Response body accumulation is still string-backed, and `read_request` remains bounded whole-request polling. These are not EventEmitter or Compartment blockers.
8. **Pre-existing test gate.** `cargo test -p rusty-js-runtime` previously tripped a pre-HS `module_golden.rs` compile error in the integration-test target. Focused `--lib caps::tests::net` lane passed (4 tests). The trajectory documents this honestly; do not waste time chasing it as an HS regression without first rechecking current `main`.
