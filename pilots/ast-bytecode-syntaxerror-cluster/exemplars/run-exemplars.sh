#!/usr/bin/env bash
# Cluster exemplar runner. Runs the 100 stratified-sample tests via
# the test262 harness wrapper against cruft; prints aggregate pass/fail
# and surface-family breakdown of remaining fails.
set -uo pipefail
...
echo "Cluster exemplars: PASS=$PASS FAIL=$FAIL / $TOTAL  ($(awk "BEGIN{printf \"%.1f\", $PASS*100/$TOTAL}")%)"
echo "--- top fails by surface family ---"
for f in "${FAILS[@]:-}"; do
  echo "$f" | awk -F/test/ '{print $2}' | awk -F/ '{print $1"/"$2}'
