# ast-bytecode-syntaxerror-cluster — Seed
...
```
ast-to-bytecode/language-lowering :: E2/internal-method:execution-semantics :: parser-form/early-error :: err:SyntaxError
```

This coordinate ranks **#2** in the full-suite Pin-Art matrix
(`pilots/test262-categorize/full-suite/results/test262-full-2026-05-25-165734-p2/matrix.md`),
**1296 fails** in the canonical full-suite run. Per the apparatus
...
**Heuristics §IV classification**: C (Parser/Early-Error) — at the lowering tier

Early-error rules that fire at AST→bytecode lowering time (post-parse). Surfaces include arguments-object strict-mode mutations, ASI restrictions, let-binding scope errors, redeclaration. Sibling to parser-early-error-residual but at a deeper rung — these are rules the parser permits but the lowering must reject.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 1296-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (1296 fails)
