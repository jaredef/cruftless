...
fn p384_p_local() -> BigUInt {
    BigUInt::from_be_bytes(&(0..48).map(|i| if i<3 { 0xff } else if i==3 { 0xff } else if i==36 { 0xfe } else if i>=37 && i<40 { 0xff } else if i>=40 && i<44 { 0x00 } else { 0xff }).collect::<Vec<u8>>())
}
fn main() {
    ...
    for _ in 0..2000 {
        ...
        for j in 0..48 { bytes[j] = (next() & 0xFF) as u8; }
        ...
        if canonical.to_be_bytes(48) != via_sol.to_be_bytes(48) { div += 1; }
    }
