...
fn emit_expr(e: &Expr) -> String {
    match e {
        ...
        Expr::Arg(i) => format!("args.get({}).cloned().unwrap_or(Value::Undefined)", i),
        ...
        Expr::IsSpecObject(v) => format!("matches!({}, Value::Object(_))", emit_expr(v)),
        Expr::SameValue(a, b) => {
            format!(
        ...
        Expr::LooseEq(a, b) => {
            format!(
                "Value::Boolean(rt.is_loosely_equal_rt(&{}, &{})?)",
                emit_expr(a),
                emit_expr(b)
            )
        }
