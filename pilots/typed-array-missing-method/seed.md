# typed-array-missing-method — Seed
...
```
runtime/buffer-typed-array :: E3/intrinsic-object:ecma-262 :: availability/missing-method-or-intrinsic :: failure/other
```

This coordinate ranks **#9** in the full-suite Pin-Art matrix
(`pilots/test262-categorize/full-suite/results/test262-full-2026-05-25-165734-p2/matrix.md`),
**469 fails** in the canonical full-suite run. Per the apparatus
...
**Heuristics §IV classification**: A/D (partial chapter) — TypedArray surface completion

Whole methods missing from TypedArray/DataView/ArrayBuffer prototypes. 23 each from ArrayBuffer.prototype + TypedArray.prototype + DataView.prototype. Implement-chapter shape for partial subsystem; inventory missing methods first, then implement in priority order per cross-test coverage.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 469-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (469 fails)
