...
//! runner. If absent, defaults to the most-recent sample under
//! `CRUFTLESS_TEST262_RESULTS_ROOT`, then the repo-local legacy results dir.
...
fn main() {
    ...
    // Rank cells.
    let mut ranked_cells: Vec<((String, String), usize)> =
        cells.iter().map(|(k, v)| (k.clone(), v.len())).collect();
    ranked_cells.sort_by(|a, b| b.1.cmp(&a.1));

    // Rank pipelines (structure-axis marginal).
    let mut ranked_pipelines: Vec<(String, usize)> =
        pipeline_freq.iter().map(|(k, &v)| (k.clone(), v)).collect();
    ranked_pipelines.sort_by(|a, b| b.1.cmp(&a.1));

    // Rank data-axis (data marginal).
    let mut ranked_data: Vec<(String, usize)> =
        data_freq.iter().map(|(k, &v)| (k.clone(), v)).collect();
    ...
    let top_15_pct = if fail > 0 {
        100.0 * top_15_files as f64 / fail as f64
    } else {
        0.0
    };
    ...
    summary.push_str(&format!(
        "- distinct (pipeline × data) cells: {}\n",
        ranked_cells.len()
    ));
    summary.push_str(&format!(
        "- top-15 cells account for **{:.1}%** of failures ({}/{})\n",
    ...
    matrix.push_str(&format!(
        "Source: `{}`. Total FAIL: **{}** ({:.1}% of runnable {}).\n\n",
        results_path.display(),
        fail,
        100.0 - pct,
        runnable
    ...
    for (i, ((pipeline, data), count)) in ranked_cells.iter().enumerate().take(40) {
        let example = cells
            .get(&(pipeline.clone(), data.clone()))
            .and_then(|v| v.first())
            .map(|(p, _)| p.clone())
            .unwrap_or_default();
        let example_rel = example
            .strip_prefix("/home/jaredef/test262/test/")
            .unwrap_or(&example)
            .to_string();
        matrix.push_str(&format!(
            "| {} | `{}` | `{}` | {} | `{}` |\n",
...
fn structure_axis_pipeline(path: &str) -> String {
    ...
    match parts.as_slice() {
        ...
        ["built-ins", obj, ..] => format!("{}", obj),
        ["language", "expressions", construct, ..] => format!("language.expressions.{}", construct),
        ["language", "statements", construct, ..] => format!("language.statements.{}", construct),
        ["language", construct, ..] => format!("language.{}", construct),
...
fn data_axis_shape(path: &str, reason: &str) -> String {
    ...
    if r.contains("typeerror") {
        tags.push("err:TypeError".into());
    } else if r.contains("rangeerror") {
        tags.push("err:RangeError".into());
    } else if r.contains("syntaxerror") {
        tags.push("err:SyntaxError".into());
    } else if r.contains("referenceerror") {
        tags.push("err:ReferenceError".into());
    } else if r.contains("uri") {
        tags.push("err:URIError".into());
    } else if r.contains("test262error") {
        tags.push("err:Test262Error".into());
    }
    if r.contains("expected") && r.contains("thrown") {
        tags.push("expected-throw-missing".into());
    }
    if r.contains("not callable") {
        tags.push("not-callable".into());
    }
    if r.contains("compileerror") {
        tags.push("err:CompileError".into());
    }
    if r.contains("module not found") {
        tags.push("module-not-found".into());
    }
...
fn find_latest_results(repo_root: &Path) -> Option<PathBuf> {
    ...
    if let Some(dir) = std::env::var_os("CRUFTLESS_TEST262_RESULTS_ROOT").map(PathBuf::from) {
        latest = latest_result_under(&dir, latest);
    }
    latest_result_under(&repo_root.join("results"), latest)
}

fn latest_result_under(results_dir: &Path, mut latest: Option<PathBuf>) -> Option<PathBuf> {
