...
impl Runtime {
    ...
    fn get_sync_iterator_for_yield_delegate(
        &mut self,
        iterable: Value,
    ) -> Result<(ObjectRef, Value), RuntimeError> {
        let receiver = match iterable {
            Value::Object(id) => id,
            Value::Undefined | Value::Null => {
                return Err(RuntimeError::TypeError(
                    "yield* cannot iterate undefined or null".into(),
                ))
            }
            ref other => match self.to_object(other)? {
                ...
                _ => {
                    return Err(RuntimeError::TypeError(
                        "yield* iterable cannot be converted to object".into(),
                    ))
                }
            },
        };
        let method = self.object_get(receiver, "@@iterator");
        if !self.is_callable(&method) {
            return Err(RuntimeError::TypeError(
                "yield* iterator method is not callable".into(),
            ));
        }
        let iterator = self.call_function(method, Value::Object(receiver), Vec::new())?;
        let iterator = match iterator {
            Value::Object(id) => id,
            _ => return Err(RuntimeError::TypeError("yield* iterator is not an object".into())),
        };
        let next = self.object_get(iterator, "next");
    ...
    fn get_own_property_descriptor_pk(
        ...
        match key {
            ...
            crate::value::PropertyKey::Symbol(rc) => self
                ...
                .cloned()
                .or_else(|| self.obj(target).get_own(rc.as_str()).cloned()),
        }
    }
    ...
    pub fn object_get_own_property_descriptor_via(
        ...
        let (has, value, writable, enumerable, configurable, getter, setter) = if is_array_length {
            ...
            match self.get_own_property_descriptor_pk(id, &key_pk) {
                Some(d) => (
                    true,
                    d.value.clone(),
                    d.writable,
    ...
    pub fn promise_all_via(&mut self, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        // PIID-EXT 1 (Promise.all): interleaved per-element iteration
        // per ECMA-262 §27.2.4.1 + PerformPromiseAll. Prior impl drained
        // the iterable into a Vec before sizing the values array; the new
        // shape grows values lazily per element. Any abrupt completion
        // during iteration calls iter_close_rt-best-effort then rejects
        // the capability with the ORIGINAL error per §7.4.9 step 4.
        let iter_v = args.first().cloned().unwrap_or(Value::Undefined);
        let _ = self.promise_all_interleave(
            iter_v,
            &ctor,
            &promise_resolve,
            &cap_resolve,
    ...
    /// PIID-EXT 1: interleaved Promise.all body. Returns Ok on completion
    /// (normal or already-rejected). Per-element shape mirrors §27.2.4.1.2
    /// PerformPromiseAll: append undefined to values; create per-index
    /// resolve_element closure; remaining++; chain .then(resolve_element,
    /// cap_reject); on iteration done, decrement remaining and possibly
    /// resolve the capability.
    fn promise_all_interleave(
        &mut self,
        iter_v: Value,
        ctor: &Value,
        ...
        self.object_set(values_arr, "length".into(), Value::Number(0.0));
        ...
        loop {
            let result = match self.call_function(
                ...
                Ok(v) => v,
                Err(e) => {
                    // IteratorNext throw -> reject capability (no close
                    // per §7.4.9 + spec commentary).
                    self.promise_reject_with_error(cap_reject, e)?;
                    return Ok(());
                }
            };
            ...
            // Append undefined to values; length += 1.
            self.object_set(values_arr, index.to_string(), Value::Undefined);
            self.object_set(
                values_arr,
                "length".into(),
            ...
            // resolve_element factory.
            let already_v = match self.cell_array_new_via(&Value::Boolean(false)) {
                Ok(v) => v,
                Err(e) => {
                    let _ = crate::intrinsics::iter_close_rt(self, iter_id);
                    self.promise_reject_with_error(cap_reject, e)?;
            ...
            let resolve_element = match crate::generated::promise_all_resolve_element_factory(
                self,
                Value::Undefined,
                &[
    ...
    pub fn promise_all_settled_via(&mut self, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        // PIID-EXT 2 (Promise.allSettled): interleaved per ECMA-262
        // §27.2.4.2 + PerformPromiseAllSettled. Same body shape as
        // Promise.all: per element appends undefined to values, increments
        // remaining, creates per-index resolve + reject element closures
        // that always fulfill the slot with {status, value/reason}.
        let iter_v = args.first().cloned().unwrap_or(Value::Undefined);
        let _ = self.promise_all_settled_interleave(
            iter_v,
            &ctor,
            &promise_resolve,
            &cap_resolve,
    ...
    fn promise_all_settled_interleave(
        &mut self,
        iter_v: Value,
        ctor: &Value,
        promise_resolve: &Value,
        ...
        loop {
            ...
            let resolve_element =
                match crate::generated::promise_all_settled_resolve_element_factory(
                    self,
                    Value::Undefined,
                    &factory_args,
                ) {
            ...
            let reject_element =
                match crate::generated::promise_all_settled_reject_element_factory(
                    self,
                    Value::Undefined,
                    &factory_args,
                ) {
    ...
    pub fn promise_any_via(&mut self, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        let (capability_promise, cap_resolve, cap_reject) = self.new_promise_capability(&ctor)?;
        // PIID-EXT 3: route C.resolve resolution + iteration through the
        // capability-rejection pattern (matching race/all/allSettled).
        let promise_resolve = match self.spec_get(&ctor, "resolve") {
            Ok(v) => v,
            Err(RuntimeError::Thrown(v)) => {
                self.call_function(cap_reject.clone(), Value::Undefined, vec![v])?;
        ...
        if !self.is_callable(&promise_resolve) {
            let msg = "Promise.any: C.resolve is not callable";
            let rejection = match crate::intrinsics::make_error_instance(self, "TypeError", msg) {
                Some(id) => Value::Object(id),
                None => Value::String(Rc::new(format!("TypeError({:?})", msg))),
            };
        ...
        let iter_v = args.first().cloned().unwrap_or(Value::Undefined);
        let _ = self.promise_any_interleave(
            iter_v,
            &ctor,
            &promise_resolve,
            &cap_resolve,
    ...
    fn promise_any_interleave(
        ...
        self.object_set(errors_arr, "length".into(), Value::Number(0.0));
        ...
        loop {
            ...
            let value = self.object_get(rid, "value");
            self.object_set(errors_arr, index.to_string(), Value::Undefined);
            self.object_set(
                errors_arr,
                "length".into(),
                Value::Number((index + 1) as f64),
            ...
            let reject_element = match crate::generated::promise_any_reject_element_factory(
                self,
                Value::Undefined,
                &[
                    Value::Number(index as f64),
        ...
        loop {
            ...
            match op {
                ...
                Op::YieldDelegate => {
                    let site_pc = frame.pc - 1;
                    if let Some(generator) = self.active_generator_for_yield {
                        let existing_delegate = match &self.obj(generator).internal_kind {
                            crate::value::InternalKind::Generator(g) => g.delegate.clone(),
                            _ => None,
                        };
                        let (delegate, args) = if let Some(delegate) = existing_delegate {
                            let sent = frame.pop().unwrap_or(Value::Undefined);
                            (delegate, vec![sent])
                        } else {
                            let iterable = frame.pop()?;
                            let (iterator, next_method) =
                                self.get_sync_iterator_for_yield_delegate(iterable)?;
                            (
                                crate::value::GeneratorDelegate {
                                    iterator,
                                    next_method,
                                },
                                Vec::new(),
                        ...
                        let (done, value) = self.yield_delegate_next(
                            delegate.iterator,
                        ...
                        if done {
                            ...
                            frame.push(Value::Undefined);
                            let mut snapshot = FrameSnapshot::from_frame(frame, None);
                            snapshot.pc = site_pc;
                            match &mut self.obj_mut(generator).internal_kind {
                                crate::value::InternalKind::Generator(g) => {
                                    g.state = crate::value::GeneratorState::SuspendedYield;
                        ...
                        let iterable = frame.pop()?;
                        for value in crate::intrinsics::collect_iterable(self, iterable)? {
                            let arr = *self.gen_yields_stack.last().ok_or_else(|| {
                                RuntimeError::TypeError("yield* outside generator".into())
                            })?;
                            let len = self.array_length(arr);
                            self.object_set(arr, len.to_string(), value);
                            self.object_set(
                                arr,
                                "length".into(),
    ...
    fn call_function_inner(
        ...
        if is_generator && !proto.is_async {
...
fn inject_return_into_frame(frame: &mut Frame<'_>) -> bool {
    let Some(t) = frame.try_stack.pop() else {
        return false;
    };
    frame.operand_stack.truncate(t.sp_at_entry);
...
mod gcs_tests {
    ...
    #[test]
    fn generator_return_runs_finally_yield_then_completes() {
        let rt = run_js_runtime(
            r#"
            function* g() {
              try {
        ...
        .expect("return should run finally before completing");
        let result = rt.global_get("result");
        assert!(matches!(&result, Value::String(s) if s.as_ref() == "1 false cleanup false done true"));
    }

    #[test]
    fn generator_return_without_finally_completes_immediately() {
        let rt = run_js_runtime(
            r#"
            function* g() {
              yield 1;
        ...
        .expect("return without finally should complete directly");
        let result = rt.global_get("result");
        assert!(matches!(&result, Value::String(s) if s.as_ref() == "1 false done true undefined true"));
    }
    ...
    fn generator_yield_star_delegates_array_values() {
        ...
        .expect("yield* should delegate array values lazily");
        let result = rt.global_get("result");
        assert!(matches!(&result, Value::String(s) if s.as_ref() == "1 false 2 false 3 false undefined true"));
    }

    #[test]
    fn generator_yield_star_propagates_inner_return_value() {
        let rt = run_js_runtime(
            r#"
            function* inner() {
              yield "a";
