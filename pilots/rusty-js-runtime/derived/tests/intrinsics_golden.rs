...
fn math_floor_ceil_round() {
    if let Value::Number(n) = run("return Math.floor(3.7);") {
        assert_eq!(n, 3.0);
    } else {
        panic!();
    }
    if let Value::Number(n) = run("return Math.ceil(3.2);") {
        assert_eq!(n, 4.0);
    } else {
        panic!();
    }
    if let Value::Number(n) = run("return Math.round(3.5);") {
        assert_eq!(n, 4.0);
    } else {
        panic!();
    }
    if let Value::Number(n) = run("return Math.round(3.4);") {
        assert_eq!(n, 3.0);
    } else {
        panic!();
    }
...
#[test]
fn math_max_min() {
    if let Value::Number(n) = run("return Math.max(1, 7, 3, 5);") {
        assert_eq!(n, 7.0);
    } else {
        panic!();
    }
    if let Value::Number(n) = run("return Math.min(1, 7, 3, 5);") {
        assert_eq!(n, 1.0);
    } else {
        panic!();
    }
...
#[test]
fn parse_int() {
    if let Value::Number(n) = run("return parseInt('42');") {
        assert_eq!(n, 42.0);
    } else {
        panic!();
    }
    if let Value::Number(n) = run("return parseInt('ff', 16);") {
        assert_eq!(n, 255.0);
    } else {
        panic!();
    }
...
fn json_stringify_primitives() {
    if let Value::String(s) = run("return JSON.stringify(null);") {
        assert_eq!(s.as_str(), "null");
    } else {
        panic!();
    }
    if let Value::String(s) = run("return JSON.stringify(true);") {
        assert_eq!(s.as_str(), "true");
    } else {
        panic!();
    }
    if let Value::String(s) = run("return JSON.stringify(42);") {
        assert_eq!(s.as_str(), "42");
    } else {
        panic!();
    }
    if let Value::String(s) = run("return JSON.stringify('hi');") {
...
fn json_parse_primitives() {
    if let Value::Number(n) = run("return JSON.parse('42');") {
        assert_eq!(n, 42.0);
    } else {
        panic!();
    }
    if let Value::Boolean(b) = run("return JSON.parse('true');") {
        assert!(b);
    } else {
        panic!();
    }
    if let Value::Null = run("return JSON.parse('null');") {
    } else {
        panic!();
    }
    if let Value::String(s) = run("return JSON.parse('\"hi\"');") {
        assert_eq!(s.as_str(), "hi");
    } else {
        panic!();
    }
