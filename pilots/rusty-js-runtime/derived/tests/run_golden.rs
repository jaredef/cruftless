...
#[test]
fn array_literal_elision_preserves_length_and_holes() {
    assert!(matches!(run("return [,,,].length;"), Value::Number(3.0)));
    assert!(matches!(run("return 0 in [,,,];"), Value::Boolean(false)));
    assert!(matches!(
        run("return [,,,].includes(undefined);"),
        Value::Boolean(true)
    ));
}
