# ast-bytecode-wrong-result — Seed
...
```
ast-to-bytecode/language-lowering :: E2/internal-method:execution-semantics :: value-semantics/wrong-result :: assertion/expected-mismatch
```

This coordinate ranks **#3** in the full-suite Pin-Art matrix
(`pilots/apparatus/test262-categorize/full-suite/results/test262-full-2026-05-25-165734-p2/matrix.md`),
**1244 fails** in the canonical full-suite run. Per the apparatus
...
**Heuristics §IV classification**: B (Shared-Upstream Bug) — highest-yield parity lane

Lowered code produces wrong runtime values per spec. Top surface family is arguments-object semantics (mapped/non-strict accessor coupling, parameter-binding shadowing). Heuristics §IV.B: pull 5+ records, identify shared abstract op, patch at highest shared resolver point.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 1244-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (1244 fails)
