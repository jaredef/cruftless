# ast-bytecode-uncategorized-projection — Seed
...
```
ast-to-bytecode/language-lowering :: E2/internal-method:execution-semantics :: uncategorized/projection :: failure/other
```

This coordinate ranks **#6** in the full-suite Pin-Art matrix
(`pilots/apparatus/test262-categorize/full-suite/results/test262-full-2026-05-25-165734-p2/matrix.md`),
**659 fails** in the canonical full-suite run. Per the apparatus
...
**Heuristics §IV classification**: APPARATUS GAP — flagged in heuristics §IX

Pin's projection is `uncategorized/projection` — apparatus says these still contain parseable runner reason structure. PRIMARY MOVE: refine the categorizer to extract sub-projection from `reason` text before spawning substrate work. Apparatus hardening, not engine fix.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 659-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (659 fails)
