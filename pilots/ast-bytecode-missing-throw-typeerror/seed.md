# ast-bytecode-missing-throw-typeerror — Seed
...
```
ast-to-bytecode/language-lowering :: E2/internal-method:execution-semantics :: abrupt-completion/throw-missing :: err:TypeError
```

This coordinate ranks **#7** in the full-suite Pin-Art matrix
(`pilots/test262-categorize/full-suite/results/test262-full-2026-05-25-165734-p2/matrix.md`),
**622 fails** in the canonical full-suite run. Per the apparatus
...
**Heuristics §IV classification**: B (Shared-Upstream Bug) — abrupt-completion gaps

Operations that should throw TypeError per spec (assign-to-undefined, primitive-coercion-of-Symbol, illegal receiver) silently succeed. Spans addition/arrow-function/computed-keys/eval-code. Likely shared at ToObject / ToPrimitive / ResolveThisBinding abstract ops.

## Apparatus

- **Exemplar suite**: `exemplars/exemplars.txt` — 100 paths stratified-
  sampled from the 622-fixture pool by surface family (proportional +
...
- large enough to matter — confirmed by pool size (622 fails)
