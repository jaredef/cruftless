...
fn structural_tag(msg: &str, src: &str) -> String {
    ...
    let near: String = if let Some(idx) = msg.find("Span { start: ") {
        let rest = &msg[idx + 14..];
        let n: usize = rest.chars().take_while(|c| c.is_ascii_digit())
    ...
    if near.contains("@") && (near.contains("class") || near.contains("@inject") || near.contains("@override")) {
    ...
    if m.contains("expected colon") { return "uncategorized-expected-colon".into(); }
    if m.contains("expected rparen") { return "uncategorized-expected-rparen".into(); }
    if m.contains("expected rbrace") { return "uncategorized-expected-rbrace".into(); }
    if m.contains("expected class member") { return "uncategorized-class-member".into(); }
    if m.contains("unexpected token") { return "uncategorized-unexpected-token".into(); }
    if m.contains("unterminated") { return "lex-unterminated".into(); }
    if m.contains("invalididentifier") { return "lex-invalid-identifier".into(); }
...
fn measure_file(path: &Path) -> Outcome {
    ...
    match res {
        ...
        Err(p) => {
            let s = if let Some(s) = p.downcast_ref::<&str>() { (*s).to_string() }
                    else if let Some(s) = p.downcast_ref::<String>() { s.clone() }
                    else { "panic (non-string payload)".to_string() };
            Outcome::Panic(s)
        }
    }
}
...
fn main() {
    ...
    let ok = results.iter().filter(|(_, o)| matches!(o, Outcome::Ok)).count();
    let strip = results.iter().filter(|(_, o)| matches!(o, Outcome::StripError(_))).count();
    let parse = results.iter().filter(|(_, o)| matches!(o, Outcome::ParseError(_))).count();
    let panic = results.iter().filter(|(_, o)| matches!(o, Outcome::Panic(_))).count();
    let success_rate = if total > 0 { 100.0 * ok as f64 / total as f64 } else { 0.0 };
    ...
    let summary = format!(
        ...
        date, total, ok, success_rate, strip, parse, panic, elapsed_ms,
        if total > 0 { elapsed_ms as f64 / total as f64 } else { 0.0 },
    );
    ...
    for (i, (tag, (count, msg))) in ranked.iter().enumerate().take(30) {
        let example = tag_examples.get(*tag)
        ...
        table.push_str(&format!(
            "| {} | `{}` | {} | `{}` | `{}` |\n",
            i + 1, tag, count, example, msg_short.replace('|', "\\|"),
        ));
    }
...
fn chrono_today() -> String {
    ...
    while remaining > 0 {
        ...
        if remaining >= year_days { remaining -= year_days; y += 1; } else { break; }
    }
