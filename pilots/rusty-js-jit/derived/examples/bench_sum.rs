...
use rusty_js_bytecode::compiler::{LocalDescriptor, FunctionProto, UpvalueDescriptor};
...
fn main() {
    ...
    println!("typed-i64 JIT mean over {} iters: {:.4}ms/call (total {:.3}ms)", iters, per_call2, elapsed2);
    println!();
    println!("=== summary ===");
    println!("interpreter:                  {:.0}ms", interpreter_baseline_ms);
    println!("JIT (plain ops, i64-cheat):   {:.3}ms  ({:.1}x speedup)", per_call, interpreter_baseline_ms / per_call);
    println!("JIT (typed-i64 ops):          {:.3}ms  ({:.1}x speedup)", per_call2, interpreter_baseline_ms / per_call2);
...
fn build_typed_i64_sum_proto() -> FunctionProto {
    ...
    FunctionProto {
        ...
        function_length: 1,
        locals: vec![
            LocalDescriptor { name: "n".to_string(), kind: rusty_js_ast::VariableKind::Let, depth: 0 },
            LocalDescriptor { name: "s".to_string(), kind: rusty_js_ast::VariableKind::Var, depth: 0 },
            LocalDescriptor { name: "i".to_string(), kind: rusty_js_ast::VariableKind::Var, depth: 0 },
        ],
