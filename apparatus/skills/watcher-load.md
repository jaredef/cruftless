...
1. **Verify the sidecar is running** at `http://127.0.0.1:7777/local/health`. If not, surface to keeper as `**[WATCHER] CONSULTATION** — sidecar not running; please start before I proceed.`

2. **Load your singleton token** from the `CAACP_TOKEN_WATCHER` env var (sourced via `scripts/env.sh`). If unset, surface to keeper — the bootstrap registration has not been completed for your role.
