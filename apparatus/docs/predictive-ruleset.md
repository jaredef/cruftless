...
**Status as of 2026-05-30**: **30 rules total; rules 1-15 + 17-30 articulated in this doc** (rule 16 does not exist in the canonical source per Doc 727 §X append-only preservation). Rules 17-22 consolidated 2026-05-30 closing audit-ledger Entries 001-004's carry-forward gap. Rules 23-26 promoted 2026-05-25 / 2026-05-28 from findings NLC.1 / IR.29 / IR.32 / IR.27. Rules 27-28 promoted 2026-05-30 from findings TAECSF.3 + APP.PIPELINE-1 at findings-disposition cycle 2 (Telegram 10600). Rule 29 promoted 2026-05-30 amending Rule 2 (Telegram 10610); companion prospective doc `apparatus/docs/measurement-determinism-2026-05-30.md`. **Rule 30 promoted 2026-05-30 amending Rule 27** via back-reference from Findings TAECSF.1 + TABSC.2 + ASTA.1 (3-instance corroboration per the findings-disposition protocol §III) at the post-ASTA-EXT 0 follow-up cycle (Telegram 10616).
...
## Rule 29 — Measurement-determinism short-circuit (amends Rule 2)

**Statement**: when a measurement instrument's first two invocations against an identical substrate state return byte-identical results (zero variance at n=2), the instrument is declared **deterministic at the current substrate maturity**; the recorded measurement is the value, not a noisy estimate. Rule 2's multi-run protocol (≥5 runs with median reporting) collapses to single-value reporting in the deterministic case. The full ≥5-runs discipline reactivates the moment any subsequent run surfaces variance > 0 at the instrument; that variance event is itself a discovery rung (the substrate's state has acquired non-determinism that the apparatus must now account for).

**Predicts**: instruments whose underlying substrate is fully reproducible (curated test262 sample with fixed seed; diff-prod with byte-identical stdout comparison; cluster exemplar runners with deterministic test262 paths) will satisfy the n=2 byte-identity condition at every substrate maturity. Insisting on ≥5 runs for such instruments wastes ~4× the measurement wall-clock without producing additional information; the saved discipline-cost is reallocable to higher-information measurement (cross-runtime bench, fresh full-suite categorize) or to substrate work. Instruments whose substrate is non-deterministic (CRB micro-benchmark with timer-noise variance; any measurement involving system-call latency or scheduler interactions) will surface variance at n=2 and remain under Rule 2's ≥5-runs discipline.

**Evidence**: 2026-05-30 test262 sample re-run (audit-ledger Entry 009) — `scripts/test262-sample/run-sample.sh` invoked twice in close temporal sequence post-TABSC-EXT 1 (commit 944a22dd). Both runs returned 6816 PASS / 865 FAIL / 16 SKIP / 88.7% byte-identical. Zero variance at n=2; the canonical 88.7% number is the measurement, not an estimate. Surfaced as candidate Rule 2 amendment in Entry 009; promoted to standing rule 2026-05-30 per keeper directive Telegram 10610. Composes empirically with the cluster-exemplar runners (TAMM, TAWR) which also produce byte-identical results across the ≥3-run stability checks the helmsman performed during the day's substrate work (e.g., the TAECSF-EXT 1 NEGATIVE diagnosis ran TAMM three times reporting 83/83/83).

**Composes with**: Rule 2 (multi-run protocol — Rule 29 amends Rule 2 via back-reference per Doc 727 §X append-only discipline; Rule 2 remains the default; Rule 29 carves the deterministic-instrument exception); Rule 3 (detectability budget — Rule 29's short-circuit does not relax Rule 3's per-N detectability thresholds, only the multi-run protocol that supplies the variance estimate; for deterministic instruments, the variance estimate is zero by construction); Rule 9 (raw-pointer cache stability — Rule 29's determinism condition is the apparatus-tier analog of Rule 9's address-stability condition); Rule 23 (founding baseline-inspection — Rule 29's "first two runs" test is a natural extension of Rule 23's at-spawn inspection discipline to the measurement instrument itself).

**Operational integration**: `apparatus/docs/measurement-determinism-2026-05-30.md` is the prospective articulation; this rule operationalizes it as a discipline edit. The audit-ledger discovery hook (Entry 009 + future-cross-locale corroborations) carries the rule's validation record. When a new instrument is added to the engagement, its first two production runs should be n=2 stability checks before being declared "canonical"; if byte-identical, the instrument enters the deterministic class and Rule 29 governs.

---

## Rule 30 — Narrow-dispatcher cascade-revival as preferred error-propagation discipline (amends Rule 27)

**Statement**: when an engine-internal non-Result-returning helper must propagate spec-mandated errors to a subset of callers (typically the user-visible / Result-aware callers, while internal infrastructure callers remain non-Result), prefer introducing a narrow Result-returning dispatcher consumed only at the Result-aware sites over lifting the helper's signature globally. The narrow dispatcher's blast radius is bounded by its consumer set; the wide signature lift's blast radius is every caller of the helper. The pattern is the engagement's preferred shape for cascade-revival of stalled error-propagation rungs.

**Predicts**: substrate rungs that lift a non-Result helper to Result without first attempting a narrow-dispatcher alternative will incur 5-10× larger blast radius than necessary. Substrate rungs that adopt the narrow-dispatcher pattern land with bounded blast radius + cleanly cascade-revive the error-propagation gap per Doc 739 (B1)/(B2)/(B3). The pattern is most effective when the abstract op being threaded is already spec-correct on its own (substrate prefix CLOSED at spawn) and only the dispatcher needs the Result-thread.

**Evidence**: Three engagement instances within 2026-05-30:
1. **TAECSF-EXT 0** (commit 1e86ed67) — `Runtime::typed_array_set_index_checked` introduced as narrow Result-returning dispatcher above `typed_array_set_index` (unchanged). BigInt-TA spec coercion propagates via `?` from the bytecode `Op::SetIndex` handler; non-Result internal callers (e.g., constructor populate paths via `object_set_pk`) continue using the unchecked helper.
2. **TABSC-EXT 0 / EXT 1** (commits f2107bb6 + 944a22dd) — `abstract_ops::number_to_raw_bytes` as substrate prefix; consumed by TA storage path inside TABSC-EXT 0 + DataView setters at TABSC-EXT 1 + cascade-revived TAECSF integer coercion at zero TAECSF-side cost. The dispatcher pattern composes with Finding TABSC.2 (substrate prefix amortization across cascade-revival rungs).
3. **ASTA-EXT 0** (commit 00a73363) — `Runtime::object_set_checked` introduced as narrow Result-returning dispatcher above `object_set` (unchanged). Array-prototype mutating-method intrinsics (pop/push/shift/unshift/splice/sort) migrated to use the checked variant with `?`; other `object_set` callers continue using the unchecked path. Probe 7/7 PASS. Three independent locales now exhibit the same pattern at three distinct dispatcher sites.

Promoted 2026-05-30 from Findings TAECSF.1 + TABSC.2 + ASTA.1 (third-instance corroboration per the findings-disposition protocol §III) at the post-ASTA-EXT 0 follow-up cycle per keeper directive Telegram 10616. The 3-locale cross-instance count exceeds the standard 2-instance promotion threshold; Rule 30 codifies the engagement's preferred error-propagation discipline.

**Composes with**: Rule 27 (substrate-spec-correctness vs engine-architecture conflict — Rule 30 amends Rule 27 via back-reference per Doc 727 §X append-only; Rule 27 names the conflict, Rule 30 names the preferred rectification shape); Rule 4 (never split a substrate move — the narrow-dispatcher pattern lands the full Result-thread in one rung, not split across many lift-then-fix sub-rungs); Rule 21 (probe-first scoping — the narrow dispatcher IS the minimum-property-defeating substrate that the probe defeats); Doc 739 (single-tier cascade-revival — Rule 30 is the engineering shape that realizes Doc 739's (P3) "the substrate move to revive a (P2.d) sub-pilot is not a sub-pilot-local move; it is an upstream constraint-closure move").

**Operational integration**: when a new substrate rung surfaces a spec-error-propagation gap, the proposal's §Substrate Moves should explicitly evaluate the narrow-dispatcher pattern (Rule 30) as the default choice; deviation to a wide signature lift requires explicit rationale citing why the narrow-dispatcher alternative fails (e.g., when ≥80% of the non-Result helper's callers actually need the Result-thread, making the wide lift's blast radius lower than maintaining two parallel call paths).
