...
**Sidecar + per-instance-token extension (Telegram 10252–10257, landed in 4 rungs 2026-05-29)**: extends the protocol with a local cybernetic-coordination sidecar at `apparatus/caacp-server/` that runs on each cruftless clone, polls jaredfoy.com per registered agent, surfaces incoming messages through three channels (α file-write, β callback URL, γ Telegram fallback deferred), and brokers outbound through a localhost HTTP API. Per-role tokens replace the shared-token model for the four appointed governance/service roles (`CAACP_TOKEN_{HELMSMAN,ARBITER,WATCHER,DEPUTY}` env vars); substrate-resolver instances register per-session and receive their own `caacp-substrate-resolver-<uuid>` tokens. jaredfoy.com endpoint extended with `POST /register` and `GET /tokens?role=X` admin-tier paths plus per-token authentication with role-binding enforcement. Agent init protocol at `apparatus/docs/agent-init-protocol.md` is the bootstrap procedure for every session. Sidecar HTTP API + operational notes at `apparatus/caacp-server/README.md`; companion wrapper at `apparatus/scripts/caacp-sidecar.sh`.
