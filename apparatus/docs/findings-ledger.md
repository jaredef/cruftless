...
- **Promotion status**: trajectory-and-findings-embedded; one-more-observation. Candidate apparatus standing-rule on substrate-spec-correctness-vs-engine-architecture conflict (when a spec-faithful substrate fix breaks because the engine architecture assumes a non-spec storage model the test cluster exploits via a harness aliasing pattern). Awaiting a second cross-locale instance.
...
- **Promotion status**: trajectory-and-findings-embedded; one-more-observation. Candidate apparatus standing-rule pending second cross-pillar instance (BigInt arithmetic, RegExp coercion, or another type-specific dynamic-language fix).
