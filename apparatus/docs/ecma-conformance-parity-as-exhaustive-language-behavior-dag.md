...
#### XI.1.b Carriers within tokenization-coordinate scope (updated post-PPIF-EXT 2)

LGSS-EXT 3 documented two carriers as irreducible within the lexical-grammar coordinate class. **One has since been eliminated** by a sibling locale that named the orthogonal constraint LGSS had identified as outside its scope.

**Current state (post-PPIF-EXT 2)**:

1. **`enter_template_tail`** — the template-substitution-close re-lex. cruft's lexer emits Template{Middle/Tail} *starting at the `}` byte itself*; the `}` is the leading delimiter of the next template part, not its predecessor. The only correct sequence is **re-lex at the same byte position** under TemplateTail goal. This is structurally distinct from bump's forward fetch and cannot be folded into a per-bump hook without restructuring lexer byte-boundaries (deep lexer change affecting raw/cooked-string semantics) or introducing a pre-bump hook (parser-machinery change). Both are outside tokenization-coordinate scope. **Status: remains the one carrier of the back-edge in cruft's current architecture.**

2. ~~**`rewind_lexer_to`**~~ — **DELETED 2026-05-25 by PPIF-EXT 2** (see `apparatus/docs/deletions-ledger.md`). The carrier was eliminated by the sibling locale `pilots/parser-precedence-in-flag/`, which named the `[+In]`/`[-In]` grammar parameter as parser state (PPIF-EXT 1) — making the bare-ident for-head fast-path that `rewind_lexer_to` served structurally redundant. PPIF-EXT 2 then deleted the fast-path + the rewind together (-48 net LOC). The carrier count for the back-edge is **1**, down from 2.

Both the surviving carrier and the eliminated one were intent-named (method name expresses parser-tier purpose) and parser-internal (no downstream tier sees `LexerGoal`). What changed is that the eliminated carrier's "irreducibility within LGSS scope" turned out to be a scope-of-LGSS observation, not a scope-of-the-substrate observation: a sibling locale operating at an adjacent coordinate dissolved the irreducibility.

**Standing finding (LGSS.5, refined post-PPIF.4)**: locale boundaries are themselves coordinates. What appears irreducible within one locale's scope may become reducible once a sibling locale names the orthogonal constraint. The rule-11 pre-spawn check should include *"what carries the locale's scope, what borders sit on other locales' scopes"* alongside the existing 5 axes. **Refinement (PPIF.4)**: when a sibling locale closes successfully and names the previously-orthogonal constraint, the prior locale's "irreducible carrier" list should be re-audited; carriers may have been promoted to reducible by the sibling's work without the prior locale noticing. The deletions ledger captures this kind of cross-locale chain explicitly.

**Spinoff locale (now closed)**: `parser-precedence-in-flag` (PPIF) implemented `[+In]`/`[-In]` threading per the prior §XI.1.b prediction. PPIF-EXT 1 named the parser-state field; PPIF-EXT 2 landed the consequent deletion. PPIF-EXT 3 (audit other for-* positions taking [-In]) remains open.

**Surviving back-edge in cruft's architecture**: lexer↔parser feedback edge carries via `enter_template_tail` only. Eliminating this would require either lexer byte-boundary restructure or a pre-bump hook in the parser-machinery; both outside tokenization-coordinate scope and currently not on any active locale.
