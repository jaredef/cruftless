...
- **Status**: PROMOTED — founded 2026-05-30 at `pilots/ta-element-coercion-spec-faithful/` via TAECSF-EXT 0 (probe selected option (ii) per the bifurcation). See ledger Entry 018 for the un-defer back-reference.
...
### Entry 018 — `ta-element-coercion-spec-faithful` PROMOTED (2026-05-30)

- **Candidate name**: `ta-element-coercion-spec-faithful` (founded locale at `pilots/ta-element-coercion-spec-faithful/`).
- **Back-reference**: Entry 010 (2026-05-28). Entry 010's Status field flipped in-place to PROMOTED per the discipline at this file's §Discipline.
- **Originating rung**: this entry — helmsman session 2026-05-30 under keeper directive Telegram 10566 ("Continue. Approved") approving proposal `apparatus/proposals/pending/2026-05-30T160500Z-taecsf-ext-0-tobigint-result-probe/proposal.md`. Action 3 of the 2026-05-30 deferrals-vs-substrate audit (audit-ledger Entry 001).
- **Un-defer signal**: probe rung TAECSF-EXT 0 selected option (ii) per Entry 010's bifurcation — narrow Result-returning TA element-set dispatcher (`Runtime::typed_array_set_index_checked` at `interp.rs:632`) routed from the bytecode `Op::SetIndex` handler. Closes the BigInt-TA element-set spec-faithfulness gap (StringToBigInt failure now throws SyntaxError per §7.1.13) without modifying `object_set_pk`'s signature. Yield at founding: TAWR 63 → 67 / 100 (+4), diff-prod 61/51 → 64/48 (+3), TAMM 82 → 86 / 100 (≥82 regression gate satisfied).
- **Founding rung**: TAECSF-EXT 0 (LANDED 2026-05-30). Carry-forward sub-substrates within the locale: (a) integer-kind ConvertNumberToTypedArrayElement per §10.4.5.16; (b) Float32 canonical-NaN preservation per §6.1.6.2; (c) cross-reference to Entry 001 for the Number→BigInt spec deviation in `abstract_ops::to_bigint`.
- **Arc enrollment**: `2026-05-28-array-exotic-substrate` sub-locale roster updated; third in-flight locale at land time alongside TAWR closed at EXT 6 and RBDPA founded at EXT 0.
- **Status**: RECORDED (this is a back-reference entry; Entry 010 carries the PROMOTED status flip).
