...
The inclusion set is canonicalized in a Claude Code skill at `apparatus/skills/arbiter-load.md` (`.claude/skills/` is a symlink to `apparatus/skills/` so Claude Code's skill discovery finds the apparatus-tracked version) that the arbiter invokes as the first action of every session.
...
**Stage 2 — Mechanical-veto tier (LANDED 2026-05-28 per keeper directive Telegram 10216).** The `.githooks/pre-push` hook is installed and active locally via `git config core.hooksPath .githooks`; the `apparatus/proposals/{pending,decided,archived}/` directory structure is created; the `apparatus/skills/arbiter-load.md` skill is authored (with `.claude/skills/` symlinked to `apparatus/skills/` per keeper directive Telegram 10219). The first arbiter session processes the proposal queue end-to-end as the protocol's first live cycle.
...
**Stage 4 — Service-tier activation (LANDED 2026-05-28 per keeper directive Telegram 10219).** Adds the two non-governance resolver roles articulated at `apparatus/docs/service-tier-and-statefulness-protocol.md`. Concretely: (a) `apparatus/skills/watcher-load.md` and `apparatus/skills/deputy-load.md` skills authored to instantiate each role with its curated context; (b) `apparatus/watcher/{notifications,notifications/closed}/` and `apparatus/deputy/{fleet-state,broadcasts}/` directory structures created with READMEs documenting the notification + summary + broadcast formats; (c) the erasure-stateful surfaces in the freshness protocol's §V.2 inventory will be annotated with `(last-refreshed: YYYY-MM-DD)` in the apparatus docs that cite them on first watcher session (the annotation discipline is load-bearing from that point); (d) the watcher's polling cadence (§VI.3 of the freshness protocol) is configured per surface in the watcher-load skill; (e) the deputy's fleet-state-summary cadence default is 30 minutes when fleet is active, configurable per the keeper's fleet-management policy. Telegram conventions extended with `**[WATCHER] INFO**` and `**[DEPUTY] INFO**` role tags. Service-tier roles do not gate pushes (Stage 2 mechanical-veto remains the only push gate); they prevent staleness drift and fleet coordination failures upstream of the push tier. Watcher and deputy sessions are not yet appointed; the scaffolding waits.
...
**Parallel deployment stream — Cybernetic Agentic Communication Protocol (CAACP)**: per keeper directive Telegram 10239–10241, a separate three-stage deployment landed alongside this operational protocol to formalize cybernetic inter-resolver communication. CAACP Stage A (articulation + apparatus/caacp/ scaffolding + role-load-skill extensions) LANDED 2026-05-29; CAACP Stages B (endpoint deployment + token provision) and C (Telegram demotion to keeper Rung-2 escalation only) pending. The CAACP integrates with this operational protocol's §II proposal+veto workflow (the helmsman authors a CAACP `request` alongside each proposal; the arbiter's decision artifact is paired with a CAACP `acknowledgment` that transitions the request to RESOLVED), removing the keeper from routine inter-role routing once Stage B lands. Full articulation at `apparatus/docs/cybernetic-agentic-communication-protocol.md`.
