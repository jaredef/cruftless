...
Both bridges poll `/local/inbox?role=<role>`, maintain a seen-cache under `apparatus/caacp-server/data/`, and inject the shared `**CAACP NEW** role=<role> count=<N> latest=<sender>/<intent>/<slug>` directive plus a quiescence contract: poll the exact role/instance inbox, resolve or explicitly block every PENDING message, re-poll after the last ack/response, and only yield once the inbox has no actionable PENDING messages. See agent-init-protocol §V for usage + the per-bridge selection rationale.
