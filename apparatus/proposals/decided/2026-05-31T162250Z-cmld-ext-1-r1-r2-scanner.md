...
- **Behavior-neutral substrate landing.** Commit `3ff3ddca` adds a new pure module (`cjs_lexer.rs`) and one `pub mod` line. The function has no caller on any engine behavior path, so test262 / sample / diff-prod are necessarily unchanged; gates_post records this honestly rather than asserting a re-run delta. Landing validated dead code before its integration rung is sound — it lets the integration's parity delta be measured in isolation.
