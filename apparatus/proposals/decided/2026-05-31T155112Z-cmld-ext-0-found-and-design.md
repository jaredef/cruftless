...
- **Design-only, no runtime risk.** Commit `a46cc047` touches only the new nested locale `pilots/cjs-esm-namespace-pipeline/cjs-module-lexer-derived/` (seed/design/trajectory) and `apparatus/locales/manifest.json` (refresh). Gates legitimately not rerun.
