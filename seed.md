...
17. **Test-cost stratification: inner-loop tests vs scheduled-burst tests.** The engagement's reference hardware (Raspberry Pi) sets an inner-loop wall-clock budget. Tests whose individual cost exceeds 30 seconds on this hardware are marked `#[ignore]` (Rust's built-in stratification primitive) so default `cargo test` runs the inner-loop set only; full coverage runs via `cargo test -- --ignored` or `./bin/run-pilots.sh --slow`. Target default-suite wall-clock: under 3 minutes.
  ...
  **Empirical record 2026-05-11 (fourteen corroborating instances).** All from the bigint/EC/RSA arithmetic cluster:
  ...
  - Differential pass: the seven `js_differential_consumer_<surface>_matches_bun` analogues of the above. Each runs the EC/bigint work twice (once under cruftless-host, once under Bun comparator), doubling the cost.
...
18. **Fourth SIPE-T threshold: substrate-standing-in-production.** The first three SIPE-T thresholds (§A8.8, §A8.9, §A8.15) named transitions in *rule* dynamics: primitive-discovery → rule-composition → rule-standing-in-production → author-side-bug-dominance. The fourth threshold, observed during the post-compaction continuation session 2026-05-12 (commits `a24ea63` → `0427f68`, 28 sub-rounds), is the next tier: **substrate-standing-in-production** — the substrate set doing the work that previously required apparatus-extension per consumer.
  ...
  **Phase classification.** §A8.18's institution is itself the M12 move per [Doc 714 §VI.3](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point): apparatus-tier consolidation at L2M-saturation. N_persist resets is not applicable — this is a meta-rule about rule-dynamics, not a basin-expansion against a specific surface. The basin is named, not extended. Reference: trajectory Status block at session-close; Doc 708 ninth amendment; cruftless commit `0427f68` (koa landing); cruftless commit `7b0750e` (status-block + SIPE-T marker added).

19. **Post-widening cascade sweep + spec-derived L2/L3 enumerator (per [Doc 714 sub-§4.c](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point)).** Two paired disciplines that make the lower-layer constraint hierarchy precomputable rather than discovered.

    **Discipline (a) — Spec-derived L2/L3 enumerator.** A static enumerator at `host/tests/fixtures/api-surface-enumerator/main.mjs` walks the documented Node + Bun + Web-platform surface and generates one micro-test per element (typeof, prototype-method presence, static-method presence). The integration test `api_surface_enumerator_reports_coverage` runs the enumerator under both cruftless-host and Bun, computes a coverage fraction per category, and diffs failures to list precisely which spec surfaces cruftless-host lacks. Per Doc 714 sub-§4.c: L2 (presence) and L3 (shape) of the constraint hierarchy are derivable from published documentation, not requiring consumer probing to discover.
...
20. **Workspace target-dir redirect requires tool-side honor or symlink bridge.** `.cargo/config.toml` redirects `build.target-dir` to `/media/jaredef/T7/rusty-bun-target` (per the Ω.5.g.2 disk-pressure intervention; the SD card lacks headroom for a ~1.7G workspace debug build). Cargo itself honors the redirect. Any tool that hardcodes `$ROOT/target/release/<bin>` does not. The catching mechanism is the tool exiting with "Build cruftless-rquickjs first: cargo build --release --bin cruftless-rquickjs" *after* a clean build has just succeeded — the false negative is the signature.
    ...
    **Empirical record.** First instance 2026-05-17: parity-measure.sh exit-with-build-prompt after a successful `cargo build --release --bin cruftless-rquickjs`. Symlinked `target → /media/jaredef/T7/rusty-bun-target` to bridge; parity-measure ran clean immediately after. host/tools/{parity-measure.sh, route2-expanded-probe.sh, route2-value-probe.sh, broaden-basket.sh, probe-builder.sh} and any future tool reading `$ROOT/target/*` are covered by the symlink. If the disk-pressure intervention is ever rolled back, the symlink and the rule both retire together.
...
21. **Tag-on-DAG: substrate moves are named by coordinate, not by sequence-letter.** The engagement's prior `Ω.5.{letter}` discipline accreted sequentially over time, which produced collisions once the alphabet exhausted (two distinct CCCCCCCC commits with different recognitions: `f3f38deb` callee-shape-probe vs `e8f5aee1` BigInt arithmetic; same shape at DDDDDDDD and EEEEEEEE; visual conflation across 7-letter NNNNNNN and 8-letter NNNNNNNN). The corrective discipline names each move by its position in the engine's structure rather than by its position in time.
    ...
    **Form.** `Ω.5.<pipeline>.<layer>.<handle>[.<seq>]` where the coordinates are drawn from the manifest at [`host/tools/dag-coordinates.json`](host/tools/dag-coordinates.json) (manifest_version 1) and the grammar is specified at [`host/tools/tag-grammar.md`](host/tools/tag-grammar.md). Pipelines per [Doc 720 §III](https://jaredfoy.com/resolve/doc/720-the-cruftless-runtime-as-a-dag-of-interconnected-pipelines-sipe-t-topology-over-the-engine-substrate) (sixteen, enumerated). Above-engine layers L0–L6 per [Doc 714 §IV sub-§4](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point). Engine-internal layers E0–E5 per [Doc 717 §V](https://jaredfoy.com/resolve/doc/717-the-apparatus-above-the-engine-boundary-the-three-projections-lifted-to-engine-substrate-and-the-pure-abstraction-point). Per [Doc 716 §II](https://jaredfoy.com/resolve/doc/716-stubs-as-named-cuts-the-three-projection-tracker-and-the-stub-alphabet-stability-conjecture), the tag is the three-projection coordinate read off the substrate node touched.
    ...
    **Phase classification.** §A8.21's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; the rule names a coordinate system over the existing substrate. N_persist resets to 0; the basin is wider by one named naming discipline. Reference: cruftless commit instituting this entry; companion artifacts `host/tools/dag-coordinates.json` (manifest v1) and `host/tools/tag-grammar.md` (grammar v1).
...
22. **Tag-on-DAG operational discipline: validator + append-only log + trajectory column (advisory tier).** The grammar instituted at §A8.21 names the coordinate system; this entry institutes the operability for it. Three artifacts plus one documentation change land together: `host/tools/tag-validate.sh` parses a tag against the manifest and exits nonzero on any malformed segment (unknown pipeline id, unknown layer id, non-kebab handle, numeric-seq smell warning per grammar §2); `host/tools/tag-append.sh` calls the validator, refuses to append on invalid input, refuses to append on a (pipeline, layer, handle) collision against a different commit, and is idempotent on identical (tag, commit) pairs; `host/tools/tag-log.jsonl` is the append-only structured record (one JSON object per line: tag, commit, ISO 8601 timestamp, pipeline, layer, handle, recognition); the trajectory.md EXT 7 table carries a `Tag-on-DAG` column with the worked-example mapping per grammar §4.
    ...
    **Phase classification.** §A8.22's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; no new alphabet element; the rule supplies operability for the coordinate system §A8.21 named. N_persist resets to 0; the basin is wider by one named operational discipline. Reference: cruftless commit instituting this entry; companion artifacts `host/tools/tag-validate.sh`, `host/tools/tag-append.sh`, `host/tools/tag-log.jsonl`, and the new trajectory.md `Tag-on-DAG` column at the EXT 7 anchor.
...
23. **host-v2 + rusty-js-runtime is the primary substrate target. The rquickjs-backed host/ binary is reference ceiling only.** Per keeper directive 2026-05-17, after Ω.5.P17 (joint await + dynamic-import lift) opened the host-v2 measurement surface, the engagement's substrate-introduction work centers on the hand-rolled engine. New substrate moves land in `pilots/rusty-js-{ast,parser,bytecode,runtime}/derived/src/` and `host-v2/src/`. The legacy `host/` binary continues to compile and continues to participate in the parity sweep as the migration-cost reference per [Doc 717 §VIII](https://jaredfoy.com/resolve/doc/717-the-apparatus-above-the-engine-boundary-the-three-projections-lifted-to-engine-substrate-and-the-pure-abstraction-point), but no new feature work lands there.
    ...
    **Operational consequence.** Substrate-move proposals name a file:line in the rusty-js crates or host-v2; rquickjs-side proposals require explicit justification (typically: closing an above-engine bug that already independently exists in host/ and surfaces in its parity readings). The parity-measure.sh harness remains the canonical measurement against `host/`; parity-measure-v2.sh (sister script that sets RB_BIN to `target/release/cruftless-host-v2`) is the canonical measurement against host-v2. Both sweeps run on the same parity-top500.txt basket so the cut-profile diff is byte-comparable per probe.

    **Phase classification.** §A8.23's institution is itself the M12 move per [Doc 714 §VI.3](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point): apparatus-tier consolidation at the moment the host-v2 measurement surface opens. N_persist resets is not applicable. The basin is named, not extended. Reference: cruftless commits `37609af1` (Ω.5.P17 substrate move), `f7ec71ed` (Ω.5.P17.E0 stdout-fd-routing), `cc283b56` (parity-measure-v2.sh wrapper + post-EXT7 sweep outputs); trajectory.md EXT 8 anchor.
...
24. **Telos has two reads. Byte-parity-with-Bun is the headline measurement; swap-without-regression is the load-bearing telos. They diverge on Bun-laxness surfaces, and the divergence is forward progress when host-v2 is the more disciplined reading.** Per keeper recognition 2026-05-17, after the post-P21 walk of the 48-package both-ERR cluster surfaced that every entry was a genuine "package unloadable" case where both engines correctly rejected with different error-wrapper classes. The probe records FAIL because the JSON differs byte-for-byte; the engines agree at the outcome level.
    ...
    **The two reads.** The headline number `host-v2 / Bun parity` measures byte-identical-shape against Bun on each package's namespace. The load-bearing telos per seed §V is "a real consumer can swap cruftless for Bun without regression," which is satisfied when consumer-observable behavior matches even if the API-edge wrapper classes differ. The first read is a sufficient signal for the second; the second tolerates divergences the first marks as gaps.
    ...
    **Phase classification.** §A8.24's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; the rule names a measurement-versus-telos distinction that was operating implicitly and now becomes explicit. N_persist resets to 0; the basin is wider by one named methodological discipline. Reference: cruftless commits instituting this entry; companion artifacts the `MATCH_OK_ERR_BOTH` harness status and the `classify-improvements.py` post-hoc classifier.
...
25. **Substrate-amortization recurs at engine tier.** §A8.13 named the substrate-amortization pattern at host-tier (one substrate-introduction round + N closure rounds reusing it): empirically observed five times in the 2026-05-11 / 2026-05-12 host-side stretches (bigint→RSA family, EC→ECDSA/ECDH family, DER→X.509+TLS family, WebSocket frame codec→JS WebSocket class, Blake2b→Argon2id family). Doc 729 §V's vertical-recursion claim predicts the same pattern recurs at engine-substrate tier within the resolver-instance #4 (PRESTO/execution) stratum. The 2026-05-18 P59 stretch observed it twice:
    - **P58.E7 async-fn-Promise** (substrate-introduction: §27.7 AsyncFunctionStart wrap step) → +6 cluster packages (execa, semantic-release, shelljs, clipboardy, libsodium-wrappers, @actions/http-client, rollup-plugin-typescript2). Single substrate move, six-package fanout.
    - **P59.E1 Symbol-typeof** (substrate-introduction: well-known Symbol → real Value::Symbol identity) → **P59.E5 GetIndex primitive-autobox closure** (+2 visible cluster packages, plus the implicit unblocking of every iterator-protocol callsite that previously failed silently). Two-move chain, same structural shape as the host-tier substrate-amortization rounds.

    **Operational rule (extends §A8.13).** Substrate-amortization staging applies at engine-tier too: when a queued surface family shares an unimplemented engine-substrate primitive (here: async-function semantics, Symbol identity), introduce the substrate in a focused round and let closure rounds drain the consumer fanout. Don't bundle substrate + many consumers in one large round; sequence them.

    **Composition with Doc 729 §V.** The vertical-recursion claim (the resolver-instance pattern recurs across levels in one stack) predicts the substrate-amortization pattern recurs across strata. Two corroborations at engine-tier in 2026-05-18 confirm the prediction at the §V-stratum-internal boundary. The pattern is now established at three tiers: corpus (Doc 710 P1), host (§A8.13), engine-substrate (this entry).

    **Phase classification.** §A8.25's institution is a Phase-2-extension micro-round per §A8.12 — apparatus-tier rule about engine-tier work. N_persist resets to 0; the basin is wider by one named cross-stratum discipline. Reference: cruftless commits `b12791ca` (P58.E7), `a9bff2f3` (P59.E1), `193c3349` (P59.E5).

26. **Doc 729 §VII.B closure stratifies across three strata.** Doc 729 §VII.B names engine-internal bilateral boundary tightening as one of three forward applications. The closure has now operated at three structurally-distinct strata within cruftless:
    - **Global stratum (P55.E1).** Thirteen compiler-emitted lowerings (`__await`, `__dynamic_import`, `__apply`, `__construct`, `__install_accessor__`, `__yield_push__`, `__yield_delegate__`, `__object_spread`, `__array_push_single`, `__array_extend`, `__destr_array_rest`, `__destr_object_rest`, `__createRegExp`) hidden behind a `Runtime::engine_helpers` fallback table. `globalThis.X` and `Object.keys(globalThis)` no longer enumerate them.
    - **Per-object sentinel stratum (P58.E1).** Seventeen `__*` internal sentinels (`__kind`, `__is_buffer__`, `__buffer_data`, `__listeners__`, `__it_src__`, `__it_idx__`) migrated from `set_own` (enumerable) to `set_own_internal` (non-enumerable). `Object.keys(typedarray)` no longer surfaces them.
    - **Value-identity stratum (P59.E1).** Fifteen well-known Symbol globals (Symbol.iterator/asyncIterator/hasInstance/toPrimitive/toStringTag/etc.) migrated from `Value::String` sentinels to real `Value::Symbol`. `typeof Symbol.iterator === "symbol"`. Implementation preserves the inner-string key so existing iterator-protocol callsites that registered `obj["@@iterator"]` continue to work via property_key's to_string coercion.

    **Operational rule.** Any engine-internal representation choice that becomes JS-observable (via enumeration, typeof, value-equality, or property-access) is auditable at all three strata. New engine internals introduced going forward must check each stratum before landing:
    1. Is it set via `set_own` and therefore JS-enumerable? → use `set_own_internal`.
    2. Is it registered via `register_global_fn`/`register_global_native` and therefore visible to `Object.keys(globalThis)` or directly callable? → use `register_engine_helper`.
    3. Does it choose a wrong-typed representation (string for symbol, object for promise, etc.)? → use the spec-correct value variant.

    **Composition with §A8.20/§A8.21.** §A8.20 names apparatus-operability discipline across stable indirections (target-dir redirect). §A8.21 names the tag-on-DAG coordinate system. §A8.26 names a third operability discipline: the three-stratum audit checklist for any engine-internal that has a JS-observable surface. Same shape (operability across indirection), different surface (engine-internal to JS-visible).

    **Phase classification.** §A8.26's institution is a Phase-2-extension micro-round per §A8.12. No new substrate; the rule names the auditable surface the existing strata define. N_persist resets to 0; basin is wider by one named operability discipline. References: cruftless commits `7c83db8b` (P55.E1), `0e2eae15` (P58.E1), `a9bff2f3` (P59.E1).

27. **Self-referential resolver at engine tier.** Doc 729 §V claims vertically-recursive directive consumption with stage-deterministic emission. The claim has now been empirically corroborated at the engine substrate tier within cruftless's own resolver call graph:
    - **P59.E3 Function constructor** invokes `evaluate_module` from a native handler at runtime. The engine's parse+compile pipeline (Doc 729 §IV resolver-instance #3, module load) is now driven by a runtime-tier value-producing operation (resolver-instance #4, PRESTO/execution).
    - **P59.E4 indirect eval** does the same. Both wrap the user's input in a synthetic source body, route through evaluate_module, capture the result via a stash global.

    **Operational rule.** When a new engine surface needs to consume a JS-source-string-as-value (eval, Function ctor, dynamic-import string forms, `new (require('vm').Script)()` etc.), the implementation pattern is fixed: assemble synthetic source → route through `evaluate_module` → capture the result via stash global → clean up the stash. This avoids duplicating the parse+compile machinery and ensures stage-deterministic emission (same source produces same artifact regardless of the calling surface).

    **Scope honestly delimited.** Direct-eval with closure capture is NOT this pattern — it requires the eval'd source to see the caller's lexical environment, which would need a Runtime-side frame stack (interp.rs:286 names the absence). The self-referential resolver pattern handles indirect-eval (global-scope) only. Direct-eval is logged as a separate engine investment.

    **Composition with Doc 729 §V.** §V's vertical-recursion claim is now corroborated at two strata within cruftless: corpus-tier (the methodology applies across engagements) and engine-substrate-tier (the resolver pattern recurs within one stack across resolver-instances #3 and #4). The two corroborations together push Doc 729 §IX Pred-729.5 (the induced property is a structural identity rather than an engineering goal) into corroborated territory — the structural shape is empirically present, not just designed-for.

    **Phase classification.** §A8.27's institution is a Phase-2-extension micro-round per §A8.12. No new substrate alphabet element; the rule names the pattern by which the runtime invokes its own parse+compile pipeline. N_persist resets to 0; basin is wider by one named architectural discipline. References: cruftless commits `1f49cda2` (P59.E3), `5cd13922` (P59.E4).

28. **Descriptor-shape discipline at every property-install site.** Every property on a built-in object has a spec-prescribed descriptor shape per ECMA §10.2.x / §20.x / §21.x — `{writable, enumerable, configurable}` triples that are NOT the language-default `{true, true, true}` produced by Op::SetProp / `set_own`. Pre-P62 cruftless installed almost every built-in property via `object_set`, which defaults to the language-default triple; consequently, *the entire descriptor surface of the runtime was misaligned with spec*, even where the property's value was correct. The 2026-05-18 P62.E1→E14 stretch surfaced this as a structural class — 19 commits, each closing one descriptor-shape sub-cluster, with cumulative slice-level lifts of 15–50 pp across Object.* and Array.* and String.* test262 chapters.

    The discipline names three install patterns by descriptor target:
    - **Frozen** `{w:false, e:false, c:false}` — built-in ctor `.prototype` slot, namespace constants (Math.PI, Number.MAX_VALUE), function-meta-props `length`. Helper: `Object::set_own_frozen` (P61.E20 / P62.E3).
    - **Non-enumerable internal** `{w:true, e:false, c:true}` — built-in proto methods, `Function.prototype.constructor` backlink, `Symbol.toStringTag` slots, engine-internal sentinels (`__kind`, `__primitive__`, `__is_buffer__`). Helper: `Object::set_own_internal` (P58.E1 / P61.E19 / P62.E2 / P62.E4).
    - **User-default** `{w:true, e:true, c:true}` — Op::SetProp / `object_set`, user-code property assignment, ctor-`prototype` for user-defined functions (modulo writable). Helper: `Object::set_own` (existing).

    **Operational rule.** Every property-install site on a built-in object must explicitly choose its descriptor shape from the three above. The default-fallthrough path (`object_set` for built-ins) is a bug surface, not a convenience — it leaks the language default into a spec-tight context. The bulk-migration tool is `register_intrinsic_method` (built-in methods get non-enumerable + correct arity + non-constructor in one call) and `register_method` (re-defaulted in P62.E2 to the same descriptor shape via set_own_internal, retrofitting ~420 install sites without per-site touch).

    **Composition with §A8.25 / §A8.26.** §A8.25 names substrate-amortization at engine tier (one substrate introduction → many closure rounds). §A8.26 names Doc 729 §VII.B's three-stratum bilateral boundary (global / per-object / value-identity). §A8.28 names the descriptor-stratum within §A8.26's per-object boundary: *which descriptor shape* the engine-internal property carries determines whether JS consumers see it as an own/spec/built-in/user property. The three discipline lines compose into a four-axis matrix (boundary stratum × descriptor shape × visibility × mutability) that fully characterizes the engine-internal-to-JS surface.

    **Substrate-amortization corroboration.** The pattern recurred in nine sub-rounds within the P62.E1→E14 stretch: each commit introduced one descriptor-shape correction (constructor backlinks non-enum, namespace constants frozen, register_method non-enum default, etc.), and each propagated to many follow-on tests without further per-test work. Specifically:
    - **P61.E19** (constructor backlinks): closed an entire enumerability cluster on Object.gOPD (+15 tests).
    - **P61.E20** (ctor.prototype frozen): closed the writable-false cluster (+14 tests).
    - **P62.E2** (register_method default flip): closed the Date.prototype.getUTC* + symmetric cluster (+15 tests on Object.gOPD).
    - **P62.E3** (Math + Number constants frozen): closed the namespace-constant cluster (+13 tests).
    Each was a single PR-sized diff; the cumulative effect lifted Object.gOPD test262 slice 69% → 92.9%, Object.create 68% → 93.4%, Object.freeze 47% → 88.6% — all in one session.

    **Doc 729 cross-check.** The descriptor-shape discipline is a sub-rule of Doc 729 §VII.B's engine-internal bilateral boundary tightening: where §VII.B names the *fact* of the boundary, §A8.28 names the *operational mechanism* by which the boundary is maintained (descriptor-shape choice at install time). Same shape as §A8.22 supplying operability for §A8.21's coordinate system.

    **Phase classification.** §A8.28's institution is a Phase-2-extension micro-round per §A8.12. No new substrate alphabet element; the rule names the discipline by which property-install sites maintain the spec-prescribed descriptor surface. N_persist resets to 0; basin is wider by one named install-time discipline. References: cruftless commits `04f912f6` (P61.E19), `a7ced7eb` (P61.E20), `31d99ca1` (P62.E2), `528b2fe1` (P62.E3), `fcb672df` (P62.E7); helper trio at `pilots/rusty-js-runtime/derived/src/value.rs:215` (set_own_internal) + `:227` (set_own_frozen) + `:185` (set_own).

29. **ToString / ToNumber / ToBoolean substrate duality at the runtime boundary.** ECMA's abstract operations (§7.1.x ToString / ToNumber / ToBoolean / ToPropertyKey / ToObject) have two valid implementation surfaces: the **pure-primitive** form (no Runtime access; operates on Value variants in isolation; lives in `abstract_ops.rs`) and the **dispatching** form (takes `&mut Runtime`; can call JS-side `@@toPrimitive` / `valueOf` / `toString` / accessor getters; throws on side-effect-throwing user code). Pre-P62 cruftless had only the pure-primitive form. Built-in methods that needed the dispatching form either silently produced "[object Object]" (when the spec required a `toString`-joined Array form) or silently produced NaN (when the spec required ToInteger on an Object with a `valueOf` method), with no error propagation. The 2026-05-18 P62.E9 / E13 stretch introduced the dispatching form for ToNumber and ToString respectively, plus a third dispatching form for **strict ToString** (throws on Symbol per §7.1.17) that the pure form cannot model.

    The discipline names four helpers on Runtime:
    - `coerce_to_string(&v)` — lenient ToString; preserves Symbol's @@sym: form (load-bearing for ToPropertyKey storage). Used by property-key dispatch sites only.
    - `to_string_strict(&v)` — strict ToString per §7.1.17; throws on Symbol. Used by `String.prototype.*` methods and other built-ins where the spec mandates the Symbol-TypeError invariant.
    - `coerce_to_number(&v)` — ToNumber per §7.1.4 + §7.1.1.1 OrdinaryToPrimitive (number hint); dispatches `@@toPrimitive("number")` → `valueOf` → `toString` for Objects, ToNumber the result. Used by `array_length`, by argument coercion in String.prototype methods, and at every other ToInteger/ToLength site.
    - `is_callable(&v)` — IsCallable per §7.2.4; checks InternalKind for Function/Closure/BoundFunction/Proxy. Used by Array.prototype.\*'s callback-validity guard and by Object.create/defineProperty's get/set-validity guard.

    **Operational rule.** When a built-in method needs to coerce its receiver / argument per the spec's abstract operation, route through the matching `coerce_*` / `is_*` helper. The pure-primitive `abstract_ops::*` form is for situations where the runtime is unavailable (compile-time constant folding, host-side helpers without `&mut Runtime`) and where Symbol / Object → primitive dispatch is not needed (e.g., key-form preservation in property storage). Mixing the two surfaces in one method is a bug shape: it produces a method that handles primitives correctly and silently mis-handles Objects.

    **Composition with §A8.28.** §A8.28 names the descriptor-shape discipline at install time. §A8.29 names the abstract-operation discipline at *runtime* — at the moment a method consumes a Value. The two together describe the engine-internal-to-JS surface across both temporal axes (install vs runtime).

    **Substrate-amortization corroboration.** The helper introductions follow the §A8.25 pattern: one substrate-introduction round + N closure rounds. P62.E9 introduced `coerce_to_number`; P62.E13/E14 reused it across 8 String.prototype methods + array_length without further substrate work. P62.E13 introduced `to_string_strict`; P62.E14 reused it across 5 more methods. Each follow-on closure round was a sub-100-LOC PR.

    **Phase classification.** §A8.29's institution is a Phase-2-extension micro-round per §A8.12 with a substrate component: four new runtime helpers (a small alphabet extension at the runtime API tier). N_persist resets to 0; basin is wider by one named runtime-coercion discipline. References: `pilots/rusty-js-runtime/derived/src/interp.rs:264` (coerce_to_string) + `:283` (to_string_strict, P62.E13) + `:300` (coerce_to_number, P62.E9) + `:223` (is_callable, P62.E5); cruftless commits `391ed7fa` (P62.E5), `43dd10aa` (P62.E9), `53f48072` (P62.E13).

30. **Brand-check discipline: every method that depends on an internal slot must reject receivers that lack it.** ECMA's built-in prototype methods are spec'd as "If O does not have a [[XData]] internal slot, throw a TypeError exception" — this is the *brand* of an exotic object. Pre-P62 cruftless's collection methods silently fell through on missing internal slots: `Set.prototype.add.call({}, 1)` would auto-create `__set_data` on the plain Object; `Number.prototype.toString.call("x")` would coerce "x" to NaN; `Promise(fn)` (no `new`) silently constructed a non-promise. The 2026-05-19 P62.E19→E25 stretch surfaced this as a structural class — eight commits across Number, Set, Map, JSON, Promise, RegExp, each closing one brand-check gap with slice-level lifts of 10–40 pp.

    The discipline names two responsibilities at every built-in-method site:
    - **Brand check at the receiver.** Methods on a wrapper-exotic prototype (Number/String/Boolean, Set/Map/WeakSet/WeakMap, Promise, Date, etc.) must look up the internal-slot sentinel on the receiver and throw TypeError when absent. Helper pattern: `match rt.unwrap_primitive(&this) { Value::Number(n) => n, _ => return Err(TypeError) }` for primitive-wrapper protos; `match rt.object_get(this, "__set_data") { Value::Object(id) => id, _ => return Err(TypeError) }` for collection protos.
    - **Brand carry-through on the prototype itself.** Per ECMA §21.1.4 / §22.1.4 / §20.3.4, *built-in prototypes are themselves exotic wrapper objects* with [[XData]] set to a default ("" / +0 / false). Brand-checked methods must accept `Number.prototype.toString()` (called on the prototype itself) and return "0", not throw. Cruftless models this with set_own_internal("__primitive__", default) on Number/String/Boolean.prototype at install time (P62.E19.2).

    **Composition with §A8.28 + §A8.29.** §A8.28 names the descriptor shape at *install* time. §A8.29 names the abstract-op surface at *runtime* (Object → primitive dispatch). §A8.30 names the *receiver-validation* surface at runtime: which receivers does a method accept, and what's the failure shape when it doesn't. The three together fully characterize the built-in method's spec surface: install-time descriptor + receiver-check + arg-coercion.

    **Substrate-amortization corroboration (§A8.25 recurrence).** Each brand-check fix unblocked clusters of tests that depend on the same internal-slot dispatch:
    - **P62.E19** (Number ctor + Number.prototype methods through coerce_to_number + brand): closed +23 Number chapter tests in one commit. **P62.E19.2** (prototype carrying [[NumberData]]=+0F sentinel) closed a 37-test regression that the brand-tightening had introduced — itself an example of the discipline's load-bearingness.
    - **P62.E23** (Set.prototype add/has/delete brand checks): closed +30 Set tests; the *expanded* round (Map+Set across all methods via Python bulk-rewrite) added +27 more.
    - **P62.E24** (Promise ctor IsConstructor + IsCallable + static-method lengths): closed +20 Promise tests.
    - **P62.E25** (RegExp.prototype @@match/@@search/@@replace/@@split): closed +30 RegExp tests via the discipline's *dual* — installing the Symbol-keyed methods that other code's brand-checked dispatch sites need to find.

    **Doc 729 cross-check.** Brand-check discipline is a sub-rule of Doc 729 §VII.B's bilateral-boundary tightening at the *receiver-acceptance* stratum (third axis below the global-/per-object-/value-identity strata named at §A8.26). Same shape: the engine surfaces what it *will* and *won't* accept across the boundary, and the discipline makes the rejection explicit instead of silent.

    **Phase classification.** §A8.30's institution is a Phase-2-extension micro-round per §A8.12. No new substrate alphabet element; the rule names the receiver-validation discipline that the existing helper trio (set_own_internal / unwrap_primitive / is_callable) supports. N_persist resets to 0; basin is wider by one named receiver-discipline. References: cruftless commits `dbde8ed2` (P62.E19), `5cc369ba` (P62.E19.2), `979ba9d4` (P62.E23), `a97bcc6f` (P62.E23 expanded), `93ea7634` (P62.E24), `1e49daa9` (P62.E25); helper `pilots/rusty-js-runtime/derived/src/interp.rs:264` (unwrap_primitive composed with brand match at each method site).

31. **SyntaxError canonical at the runtime-error enum.** ECMA names four user-throwable error classes: TypeError, RangeError, ReferenceError, SyntaxError. Pre-P62 cruftless's RuntimeError enum carried only the first three; SyntaxError-shaped errors (JSON.parse malformed input, RegExp ctor bad pattern, Function ctor bad source) were emitted as TypeError, breaking test262's `assert.throws(SyntaxError, ...)` invariant. P62.E22 added the variant and wired it through the catch-dispatch + make_error_instance proto-chain.

    **Operational rule.** Every site that emits a runtime error chooses one of the four canonical classes. The choice is structural, not stylistic: spec sections that say "throw a SyntaxError" must produce a SyntaxError-instance; the `e instanceof SyntaxError` test262 invariant depends on the choice. JSON.parse alone has 15 such sites (every internal-grammar-violation path); each was migrated in one commit.

    **Composition with §A8.30.** §A8.30 names *what receivers* a method accepts. §A8.31 names *what error class* a method throws. Both are surface-shape disciplines on the same axis (the spec's normative throw obligations).

    **Phase classification.** §A8.31's institution is a Phase-2-extension micro-round per §A8.12 with a small substrate component: one new enum variant and its catch-dispatch wiring. N_persist resets to 0; basin is wider by one named error-class discipline. References: cruftless commit `ef9118a8` (P62.E22); `pilots/rusty-js-runtime/derived/src/interp.rs:14` (RuntimeError enum) + `:766` (catch dispatch).

32. **ToPrimitive at the operator boundary: Op::Add / Op::Eq / Op::Ne route through Runtime helpers.** ECMA §13.15 / §7.2.13 specify that `+` and `==` dispatch ToPrimitive on Object operands. Pre-P62 cruftless's Op::Add called `abstract_ops::op_add(&l, &r)` (pure-primitive, no Runtime access), so `obj + 0` evaluated to NaN regardless of obj's valueOf, and `obj == 0` evaluated to false regardless of obj's valueOf. P62.E21 introduced three Runtime-tier helpers (to_primitive, op_add_rt, is_loosely_equal_rt) and rerouted the three bytecode ops through them.

    **Operational rule (extends §A8.29).** §A8.29 named the two-surface duality (pure-primitive in abstract_ops vs dispatching on &mut Runtime) for ToString/ToNumber/IsCallable. §A8.32 names the same duality for ToPrimitive at the *operator* level (not just method-arg coercion). Three more dispatching helpers join the family. The pattern: when a bytecode op or built-in method needs Object → primitive coercion *and* a Runtime-side dispatch is available, route through the *_rt variant.

    **Substrate-amortization at the operator boundary.** This is a different amortization shape than §A8.25's "introduce substrate → drain consumers across N rounds." Here a *single bytecode op* change amortizes across *every loose-eq site in the engine and corpus*: every `obj == X`, every `X + obj`, every `if (obj == val)`. The benefit doesn't surface only in the immediate slice (Number gained +1 from this fix); it surfaces in the *long tail* of every consumer test that uses loose-eq or concat against an object that has a meaningful valueOf or toString.
...
**M12. Basin-expansion-at-L2M-saturation.** When a long-running engagement session reaches the L2M-saturation point at the session tier (per [Doc 700 Appendix C](https://jaredfoy.com/resolve/doc/700-l2m-resolved-against-the-corpus-bipartite-mutual-information-scaling-as-empirical-grounding-for-the-pin-art-channel-ensemble) and the diminishing-per-round-productive-surface signal), the next move is not "push another substrate round." It is to re-read the engagement at the lattice-extended scope per [Doc 572](https://jaredfoy.com/resolve/doc/572-the-lattice-extension-of-the-ontological-ladder) and concentrate productive surface at the artifact and corpus tiers where the L2M-bound does not constrain in the same way.
  ...
  3. Articulate the engagement's contribution to the corpus's *standing apparatus* in a corpus-tier doc (per [Doc 714](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point)'s shape for the cruftless case).
  ...
  Per [Doc 714 §VI.3](https://jaredfoy.com/resolve/doc/714-the-cruftless-engagement-read-through-the-lattice-extension-basin-expansion-at-the-l2m-saturation-point) Fal-714.3: track future long-session engagements for the predicted artifact-tier consolidation pattern. This rule is the candidate apparatus discipline the cruftless thirtieth-round saturation produced; it is offered for the corpus's standing apparatus, not just this engagement.
...
Three cybernetic compensation rules govern progress toward completion and prevent drift:
  ...
  - **§IV.M8** closes the level-2 loop for divergence-reconciliation: every divergence between cruftless and the comparator runtime must be reconciled in the round it is discovered, not deferred.
  - **§IV.M9** prevents the divergence in the first place: Tier-J fixtures are authored spec-first against Bun, not against cruftless's current surface, so divergences surface during authoring and reconciliations land in the same commit.
...
**Sub-criterion 5 — Differential testing against Bun-using applications.** The operational form of plug-and-play. For a representative basket of Bun-using applications (frameworks like Hono / Elysia, real-world apps): run `npm test` under Bun → record P_bun. Run under integrated cruftless → record P_drv. Diff. **Zero regressions across the basket** = real plug-and-play.

Closure of this sub-criterion is **per-fixture differential**: a fixture counts toward sub-criterion 5 only when it produces byte-identical output under Bun and cruftless-host (the J.1.a category in the trajectory's Tier-J basket). Fixtures that exercise the apparatus but depend on cruftless-only shapes (J.1.b) are host-internal regression tests, not sub-criterion-5 evidence — they must either be reconciled per §IV.M8 to enter J.1.a, or remain explicitly out of the differential count with a re-open condition.
...
A complementary signal: run Web Platform Tests against the integrated runtime via `wpt run` adapter. WPT pass-rate per surface is a published number for browser engines and Bun itself; cruftless's WPT pass-rate becomes an operational comparison.
...
17. **First real package: hono** — micro web framework, native Bun support. Vendor it unchanged, run its tests under cruftless-host vs Bun. Any divergence is sub-2 work.
